<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:jdf="http://www.CIP4.org/JDFSchema_2_0" xmlns:bambi="www.cip4.org/Bambi">
...
<table>
<tr height="10%">
<xsl:apply-templates select="PhaseAction"/>
</tr>
</table>


</xsl:if> <!-- if loaded job -->

</xsl:template>
<!--   ///////////////////////////////////////////////// -->

<xsl:template match="PhaseAction">
<td>
<form>
  ...
  <input type="hidden" name="DeviceStatus" >
    <xsl:attribute name="value"><xsl:value-of select="@DeviceStatus"/></xsl:attribute>
  </input>
  <input type="hidden" name="DeviceStatusDetails">
    <xsl:attribute name="value"><xsl:value-of select="@DeviceStatusDetails"/></xsl:attribute>
  </input>
  <input type="hidden" name="NodeStatus" >
    <xsl:attribute name="value"><xsl:value-of select="@NodeStatus"/></xsl:attribute>
  </input>
  <input type="hidden" name="NodeStatusDetails">
  ...
  <input type="submit" size="50">
    <xsl:attribute name="value"><xsl:value-of select="@DescriptiveName"/></xsl:attribute>
  </input>
</form>
</td>
</xsl:template>
