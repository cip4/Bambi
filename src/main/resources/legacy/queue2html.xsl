...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:jdf="http://www.CIP4.org/JDFSchema_1_1" xmlns:bambi="www.cip4.org/Bambi">
		...
		<html>
			...
			<body>
				...
				<div class="container">
								...
								<li>
									<a>
										<xsl:attribute name="href">
											<xsl:value-of select="$context"/>/overview</xsl:attribute>Device List</a>
								</li>
								<li>
									<a>
										<xsl:attribute name="href">
											<xsl:value-of select="$context"/>/showDevice/<xsl:value-of select="@DeviceID"/>
										</xsl:attribute>Device: <xsl:value-of select="@DeviceID"/>
									</a>
								</li>
								<li>
									Queue
					...
					<div class="row pt-2">
						<div class="col-12">
							<h1>Queue Device <xsl:value-of select="@DeviceID"/>
							</h1>
					...
					<div class="row pt-4">
						<div class="col-4">
							<p>
								<b>Status: <xsl:value-of select="@Status"/>
								</b>
							</p>
							<table class="table table-bordered table-sm">
								...
								<tbody>
									<tr>
										<td align="left" title="current size of queue">All Current</td>
										<td align="center">
											<xsl:value-of select="@TotalQueueSize"/>
										</td>
									</tr>
									<tr>
										<td align="left" title="size of queue including removed historic entries">Total</td>
										<td align="center">
											<xsl:value-of select="@bambi:TotalEntryCount"/>
										</td>
									</tr>
									<tr>
	...
	<xsl:template name="columnTitle">
		...
		<a>
			<xsl:attribute name="href">
				<xsl:value-of select="@Context"/>/showQueue/<xsl:value-of select="@DeviceID"/>?SortBy=<xsl:value-of select="$attName"/>
				<xsl:if test="@filter">&amp;filter=<xsl:value-of select="@filter"/>
				</xsl:if>
			</xsl:attribute>
